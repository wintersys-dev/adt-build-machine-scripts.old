...
export BUILD_MACHINE_IP="`${BUILD_HOME}/helpers/GetBuildMachineIP.sh`"

# Set up the runtime directory this is where data and information will be stored that is generated at runtime
if ( [ ! -d ${BUILD_HOME}/runtime ] )
then
	/bin/mkdir ${BUILD_HOME}/runtime 
fi
...
${BUILD_HOME}/initialisation/InitialiseLongLastingConnection.sh

#Set up anything that we would like to set for the code editors we are using
${BUILD_HOME}/initialisation/InitialiseEditor.sh
...
${BUILD_HOME}/installation/InstallCoreSoftware.sh ${BUILDOS} 
...
if ( [ "`${BUILD_HOME}/helpers/IsHardcoreBuild.sh`" != "1" ] )
then
	${BUILD_HOME}/selection/SelectCloudhost.sh ${BUILDOS}
	CLOUDHOST="`/bin/cat ${BUILD_HOME}/runtime/ACTIVE_CLOUDHOST`"
else
	${BUILD_HOME}/installation/InstallCloudhostTools.sh ${CLOUDHOST} ${BUILDOS} 
...
${BUILD_HOME}/initialisation/InitialisePrivateNetworking.sh ${CLOUDHOST}

#Run some arbitrary compatibility checks
${BUILD_HOME}/initialisation/InitialiseCompatibilityChecks.sh
...
# in the file ${BUILD_HOME}/runtime/ACTIVE_BUILD_IDENTIFIER
${BUILD_HOME}/selection/SelectBuildIdentifier.sh 
...
if ( [ ! -d ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs ] )
then
	/bin/mkdir -p ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs
fi

if ( [ "`/usr/bin/find ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs -name "*build*" -type f`" != "" ] )
then

	if ( [ ! -d ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/archive ] )
	then
		/bin/mkdir -p ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/archive
	fi

	/bin/mv ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/*build* ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/archive
fi

out_file="build_output_stream-`/bin/date | /bin/sed 's/ //g'`"
exec 1>>${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/${out_file}
err_file="build_error_stream-`/bin/date | /bin/sed 's/ //g'`"
exec 2>>${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/${err_file}
status_file="build_status_stream-`/bin/date | /bin/sed 's/ //g'`"
exec 4>>${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/${status_file}

# Tell the user where the logging output can be found
status "The main output log file is located at ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/${out_file}"
status "The main error log file is located at ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/logs/${err_file}"
status "To refer to the logs you can run the script ${BUILD_HOME}/Log.sh"
status "Press <enter> to acknowledge"

# We store the entire environment in a file ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/build_environment
# We then reference this file for variable values anywhere we need using the scripts
#
# ${BUILD_HOME}/helpers/GetVariableValue.sh
# ${BUILD_HOME}/helpers/SetVariableValue.sh
#
...
if ( [ "`${BUILD_HOME}/helpers/IsHardcoreBuild.sh`" != "1" ] || [ -f /root/PARAMETER ] )
then
	if ( [ "`${BUILD_HOME}/helpers/IsHardcoreBuild.sh`" != "1" ] )
	then
		read x
	...
	if ( [ -f /root/PARAMETER ] )
	then
		/usr/bin/env > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/build_environment 2>/dev/null
	fi
	...
	template_name="`/bin/cat ${BUILD_HOME}/runtime/current_template_name`"
	. ${template_name}
...
if ( [ -f ${BUILD_HOME}/runtime/BUILDMACHINEPORT:* ] )
then
	/bin/rm ${BUILD_HOME}/runtime/BUILDMACHINEPORT:*
fi
/bin/touch ${BUILD_HOME}/runtime/BUILDMACHINEPORT:${SSH_PORT}

export WEBSITE_DISPLAY_NAME="`/bin/echo ${WEBSITE_DISPLAY_NAME} | /bin/sed "s/\'//g" | /bin/sed 's/ /_/g'`"
${BUILD_HOME}/initialisation/InitialiseDirectoryStructure.sh ${CLOUDHOST} ${BUILD_IDENTIFIER} 
...
${BUILD_HOME}/selection/SelectSMTPSettings.sh 

# Set up the credentials for the server user
${BUILD_HOME}/initialisation/InitialiseServerUserCredentials.sh 

# Initialise/configure the datastore ready for use (access keys, tokens, host base values and so on)
#${BUILD_HOME}/initialisation/InitialiseDatastoreConfig.sh
${BUILD_HOME}/services/datastore/InitialiseDatastoreConfig.sh

if (  [ "${BUILD_FROM_SNAPSHOT}" = "1" ] )
then
	export SNAPSHOT_ID="`${BUILD_HOME}/initialisation/InitialiseSnapshots.sh`"
	/usr/bin/env > ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/build_environment 2>/dev/null
fi

${BUILD_HOME}/initialisation/InitialiseUniqueConfigDatastore.sh

# Make a few pre-flight checks to check that we are good to go
${BUILD_HOME}/initialisation/PreFlightChecks.sh 
...
then
	...
	if ( [ "`${BUILD_HOME}/services/server/CheckBuildMachineVPC.sh ${CLOUDHOST} ${BUILD_MACHINE_IP}`" = "" ] )
	then
	...
	/bin/mkdir -p ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}
	/bin/touch ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/VPC-ACTIVE
else
	if ( [ -f ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/VPC-ACTIVE ] )
	then
		/bin/rm ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/VPC-ACTIVE
	fi
fi

# Output any pre-processing messages
${BUILD_HOME}/processing/PreProcessingMessages.sh
...
then
	...
	status ""
	status "`/bin/grep -v '^#' ${BUILD_HOME}/configuration/firewall.dat`"
	status ""
	status "If your firewall is not set correctly the connection to your webserver/reverse proxy will timeout"
	status "If you are sure this configuration is what you want press <enter> otherwise you can modify the file ${BUILD_HOME}/configuration/firewall.dat"
	read x
fi
...
if ( [ "`/bin/grep "^BUILDCHAINTYPE:*" ${BUILD_HOME}/configuration/software.dat | /usr/bin/awk -F':' '{print $NF}'`" = "webserver" ] )
then
...
if ( [ "`/bin/grep "^BUILDCHAINTYPE:*" ${BUILD_HOME}/configuration/software.dat | /usr/bin/awk -F':' '{print $NF}'`" = "database" ] )
then
	${BUILD_HOME}/buildservers/BuildDatabase.sh
fi

# Put out any post processing messages to the user
${BUILD_HOME}/processing/PostProcessingMessages.sh


if ( [ ! -f ${BUILD_HOME}/runtime/BUILDING_ON_LAPTOP ] )
then
	...
	${BUILD_HOME}/services/cron/InitialiseCrontabs.sh

    #Set the hostname of the build machine
    if ( [ "`/usr/bin/hostname`" != "build-machine" ] )
    then
	    /bin/sed -i '/^127.0.0.1/a 127.0.0.1          build-machine' /etc/hosts
...
SERVER_USER="`${BUILD_HOME}/helpers/GetVariableValue.sh SERVER_USER`"
SERVER_USER_PASSWORD="`${BUILD_HOME}/helpers/GetVariableValue.sh SERVER_USER_PASSWORD`"
...
status "SERVER USERNAME :  ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/SERVERUSER"
status "SERVER PASSWORD :  ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/credentials/SERVERUSERPASSWORD"
status "###################################################################################################################"
