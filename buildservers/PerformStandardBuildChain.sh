...
DEVELOPMENT="`${BUILD_HOME}/helpers/GetVariableValue.sh DEVELOPMENT`"
BUILD_ARCHIVE_CHOICE="`${BUILD_HOME}/helpers/GetVariableValue.sh BUILD_ARCHIVE_CHOICE`"
NO_AUTOSCALERS="`${BUILD_HOME}/helpers/GetVariableValue.sh NO_AUTOSCALERS`"
NO_WEBSERVERS="`${BUILD_HOME}/helpers/GetVariableValue.sh NO_WEBSERVERS`"
IN_PARALLEL="`${BUILD_HOME}/helpers/GetVariableValue.sh IN_PARALLEL`"
BUILD_ARCHIVE_CHOICE="`${BUILD_HOME}/helpers/GetVariableValue.sh BUILD_ARCHIVE_CHOICE`"
...
DB_INSTALL_MODE="`${BUILD_HOME}/helpers/GetVariableValue.sh DB_INSTALL_MODE`"
...
NO_AUTHENTICATORS="`${BUILD_HOME}/helpers/GetVariableValue.sh NO_AUTHENTICATORS`"
...
then
	...
	then
		...
		do
			...
			${BUILD_HOME}/buildservers/BuildAutoscaler.sh ${tally}
		done
...
then
	...
	ws_active_ip="`${BUILD_HOME}/services/server/GetServerIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
...
then
	...
	ws_active_ip="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "ws-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
	db_active_ip="`${BUILD_HOME}/services/server/GetServerPrivateIPAddresses.sh "db-${REGION}-${BUILD_IDENTIFIER}" "${CLOUDHOST}"`"
fi
...
then
	...
	/bin/touch ${BUILD_HOME}/runtime/${CLOUDHOST}/${BUILD_IDENTIFIER}/PRIME_FIREWALL
fi
